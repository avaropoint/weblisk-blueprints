...
Collaboration contracts (above) govern data exchange between parties.
Operational data contracts govern an agent's relationship with its
own storage — declaring what resources the agent accesses during
normal operation and enabling the enforcement layer to verify access
at runtime.

An agent's manifest declares its capabilities (`storage:read`,
`storage:write`, etc.), but capabilities alone are too broad — an
agent with `storage:read` can read ANY resource within its scope.
Operational data contracts narrow this: an agent declares which
specific resources it accesses and what operations it performs on
each. The enforcement storage proxy verifies every operation against
this declaration.

### Purpose

1. **Least-privilege storage access** — agents declare the minimum
   set of resources they require, not just the operation types.
   The enforcement layer rejects access to undeclared resources.

2. **Runtime verification** — the enforcement storage proxy checks
   every storage operation against the agent's operational data
   contract. An agent that reads a resource it has not declared
   triggers a `data_contract_violation`.

3. **Audit and visibility** — operational data contracts make agent
   storage behavior inspectable. Operators can review what an agent
   declares it needs vs. what it actually accesses.

4. **Drift detection** — when an agent's storage access pattern
   diverges from its declared contract, the enforcement behavior
   analyzer detects the drift as a pattern deviation signal.

### Declaration

Agents declare operational data contracts in the manifest's
`contracts` section using a `storage` block:
...
contracts:
  ...
  storage:
    declared_resources:
      - resource: reports
        operations: [read, create, modify]
        scope: internal
        description: SEO scan reports generated by this agent
      - resource: scan_config
        operations: [read]
        scope: internal
        description: Configuration data loaded at startup
      - resource: scan_history
        operations: [read, create]
        scope: confidential
        description: Historical scan records with customer context
```
...
The enforcement storage proxy (see `architecture/enforcement`)
verifies every storage operation against the agent's operational
data contract:

1. Resolve the target resource from the storage operation
2. Match the target resource against the agent's declared resources
3. If no matching declaration → DENY with
   ENFORCEMENT_DATA_CONTRACT_VIOLATION
4. Verify the operation type is in the declaration's `operations`
5. If operation type not declared → DENY with
   ENFORCEMENT_DATA_CONTRACT_VIOLATION
6. If `columns` are declared, verify the operation targets only
   declared columns
7. If `row_scope` is declared, verify the operation targets only
   rows within the declared scope

### Operational Data Contract Rules

1. Agents that declare operational data contracts have their storage
   access constrained to declared resources only.
2. Agents that do NOT declare operational data contracts operate in
   capability-only mode — enforcement checks capability and scope
   but not specific resource access. This is legacy mode.
3. For scope >= `confidential`, operational data contracts are
   REQUIRED. Agents accessing confidential or higher data MUST
   declare which resources they access.
4. Operational data contracts are versioned with the agent manifest.
   Changes to declared resources follow the same change management
   process as capability changes.
5. The enforcement layer logs every storage operation with the
   matched (or unmatched) contract declaration for audit purposes.
...
- [ ] Agents accessing scope >= confidential data declare operational data contracts with specific resources
- [ ] Enforcement storage proxy verifies storage operations against declared operational data contracts
- [ ] Undeclared resource access by agents with operational data contracts triggers ENFORCEMENT_DATA_CONTRACT_VIOLATION
