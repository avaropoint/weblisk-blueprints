...
**Optional, and one approach among several.** A hub is defined by
`protocol/spec.md` and proven by `architecture/testing.md`; it is NOT defined by
this manifest. An implementation written by hand, generated by different
tooling, or produced in a language with no platform blueprint at all is equally
valid the moment it serves the protocol and passes conformance. Nothing here
narrows that.

What the manifest does is make ONE route repeatable: generating a base hub for
this platform, the same way every time, from the blueprints. It exists because a
single request for a whole implementation has no checkpoint, no attributable
failure, and no progress — and because the file set should be the blueprint's
decision rather than the model's.

Where it is used, generation becomes a loop over `files`, one call per file:
...
- `must_define` and `must_serve` are checked BEFORE conformance runs, so a
  missing symbol is found in seconds rather than as a protocol failure minutes
  later
...
endpoints are fixed here, and behaviour is fixed by the conformance suite.
Anyone preferring a different structure should change or ignore the manifest —
the contract they must meet is the protocol, not this file list.

> **Status: not yet exercised.** This manifest is derived from the layout stated
> above rather than from a generation run. Only `platforms/go.md` has been used
> in practice. Treat the file list here as a starting point to correct, not as a
> tested recipe.
...
generate:
  orchestrator:
    ...
    files:
      - path: Cargo.toml
        purpose: Crate manifest and dependencies
      - path: src/main.rs
        purpose: Entry point — configure and start the orchestrator
        must_define: [main]
      - path: src/orchestrator.rs
        purpose: HTTP routing, registration, channels, audit
        must_define: [Orchestrator]
        must_serve:
          - POST /v1/register
          - DELETE /v1/register
          - GET /v1/services
      ...
      - path: src/handlers.rs
        purpose: Handler functions, one per endpoint
    conformance: [L1]
```
