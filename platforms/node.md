...
Prepare: `npm install`
Build: `npm run build`
Run: `npm start -- --port 9800`

The `--` is part of the run command, not decoration: npm keeps flags for
itself unless they sit past a separator, so without it the port never reaches
the server.
